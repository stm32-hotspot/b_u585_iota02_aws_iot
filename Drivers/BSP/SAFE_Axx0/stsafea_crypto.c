...
static StSafeA_ResponseCode_t StSafeA_MAC_SHA_PostProcess(StSafeA_Handle_t *pStSafeA)
{
      ...
      (uint8_t)STSAFEA_CMD_HEADER_RMACEN)
  {
    ...
    for(uint8_t i =0 ; i < STSAFEA_MAC_LENGTH;i++)
    {
        a_rmac[i] = pStSafeA->InOutBuffer.LV.Data[(pStSafeA->InOutBuffer.LV.Length) + i ];
    }
    ...
    for(uint8_t i =0 ; i < STSAFEA_MAC_LENGTH;i++)
    {
        pStSafeA->InOutBuffer.LV.Data[(pStSafeA->InOutBuffer.LV.Length) + i ] =   a_rmac[i];
    }

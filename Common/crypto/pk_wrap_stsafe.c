...
/* STSAFE */
static int stsafe_ecdsa_sign_wrap( void *ctx, mbedtls_md_type_t md_alg,
                   const unsigned char *hash, size_t hash_len,
                   unsigned char *sig, size_t sig_size, size_t *sig_len,
                   int (*f_rng)(void *, unsigned char *, size_t), void *p_rng );
...
static int stsafe_eckey_sign_wrap( void *ctx, mbedtls_md_type_t md_alg,
                                   const unsigned char *hash, size_t hash_len,
                                   unsigned char *sig, size_t sig_size, size_t *sig_len,
                                   int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    if( ( ret = mbedtls_ecdsa_from_keypair( &ecdsa, ctx ) ) == 0 )
        ret = stsafe_ecdsa_sign_wrap( &ecdsa, md_alg, hash, hash_len,
                                      sig, sig_size, sig_len,
                                      NULL, NULL );
...
static int stsafe_eckey_check_pair( const void *pub, const void *prv,
                                    int (*f_rng)(void *, unsigned char *, size_t),
                                    void *p_rng )
{
    return( 0 );
                                   ...
                                   int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    uint8_t             RSLength = STSAFEA_XYRS_ECDSA_SHA256_LENGTH;
    ...
    mbedtls_ecp_keypair *ec_key_ptr = ctx;

    unsigned char       buf[ MBEDTLS_ECDSA_MAX_LEN ] = { 0 };
    unsigned char       *chr_ptr = buf + sizeof( buf );
    ...
    *sig_len = 0;
    ...
    if ( ( md_alg != MBEDTLS_MD_SHA256 ) || ( hash_len < STSAFEA_XYRS_ECDSA_SHA256_LENGTH ) )
    {
      return ( ret );
    }

    /* Check the ecc key size in private key slot using Query command with hash length */
    if ( ec_key_ptr->grp.id != MBEDTLS_ECP_DP_SECP256R1 )
    {
      return ( ret );
    }

    /* STSAFE-A ECDSA signature generation function call for Slot 0*/
    if ( SAFEA1_ECDSA_Sign( 0, (uint8_t*)hash, STSAFEA_SHA_256, STSAFEA_XYRS_ECDSA_SHA256_LENGTH, &SignR, &SignS ) != STSAFEA_OK )
    {
    ...
    else
    {
      ...
      if ( ret == 0 )
      {
        /* Note: As the mbedtls_ans1_write_mpi() writes from back of the buffer */
          /* Expected signature format is [R][S], hence writing [S] then [R] */
        MBEDTLS_ASN1_CHK_ADD( *sig_len, mbedtls_asn1_write_mpi( &chr_ptr, buf, &S ) );
        MBEDTLS_ASN1_CHK_ADD( *sig_len, mbedtls_asn1_write_mpi( &chr_ptr, buf, &R ) );

        MBEDTLS_ASN1_CHK_ADD( *sig_len, mbedtls_asn1_write_len( &chr_ptr, buf, *sig_len ) );
        MBEDTLS_ASN1_CHK_ADD( *sig_len, mbedtls_asn1_write_tag( &chr_ptr, buf, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE ) );

        if ( ( ret >= 0 ) && ( *sig_len <= sig_size ) )
        {
          memcpy( sig, chr_ptr, *sig_len );

          /* All is correct, return with 0 */
          ret = 0;
        }
        else
        {
...
const mbedtls_pk_info_t mbedtls_ecdsa_info = {
    ...
    ecdsa_can_do,
    ecdsa_verify_wrap,
    stsafe_ecdsa_sign_wrap, // STSAFE
#if defined(MBEDTLS_ECP_RESTARTABLE)
    NULL, // ecdsa_verify_rs_wrap,
    NULL, // ecdsa_sign_rs_wrap,
#endif
    NULL,
    NULL,
    stsafe_eckey_check_pair,   // STSAFE
    ecdsa_alloc_wrap,
    ecdsa_free_wrap,
#if defined(MBEDTLS_ECP_RESTARTABLE)
    NULL, // ecdsa_rs_alloc,
